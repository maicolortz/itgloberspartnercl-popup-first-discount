...
/*
import { ModalTrigger, ModalContent, CloseButton, BaseModal } from "vtex.modal-layout"; */
...
const FirstDiscount = ({ logoSrc, bannerSrc, titleDiscount, discount }: Props) => {
  const { data /* , loading */ } = useQuery(QUERY_VALUE)
  const sesion = useQuery(SESION);
  const sesion2 = useLazyQuery(SESION);

  const { isMobile } = useDevice();


  const CSS_HANDLES = ["container", "modal__overlay", "modal__box", "modal__header", "modal__close", "info", "banner", "columna__one",
